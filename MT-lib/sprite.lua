...
MTsprite = {}
function MTsprite.manipulate(spr0,ccpos,size) -- for simplicity for irregular shapes, size is to be taken as the width of the whole sprite, ccpos is assumed to be scaled already
...
function MTsprite.manipulateS(spr0,ccpos,scale) -- for simplicity for irregular shapes, ccpos is assumed to be scaled already, takes scale
	local sprite = table.deepcopy(spr0)
	if sprite.layers then
		for _,spr in pairs(sprite.layers) do
			if not spr.shift then
...
function MTsprite.add(spr1,spr2) -- adds spr2 sprite/s into spr1
	if spr1.layers then
		local sprite = table.deepcopy(spr1)
		if spr2.layers then
			for spr in pairs(spr2.layers) do
				table.insert(sprite.layers,spr)
			end
		else
			table.insert(sprite.layers,spr2)
		end
		return sprite
	else
		local sprite = {layers={}}
		table.insert(sprite.layers,spr1)
		if spr2.layers then
			for spr in pairs(spr2.layers) do
				table.insert(sprite.layers,spr)
			end
